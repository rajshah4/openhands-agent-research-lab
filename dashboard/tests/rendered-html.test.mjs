...
test("includes the Agent Canvas deployment evidence", async () => {
  ...
  assert.match(source, /These were bounded integration and recovery tests/);
  assert.match(source, /It was not one uninterrupted 20-attempt run/);
  assert.match(source, /We ran{" "}\s+\{snapshot\.controllerLoad\.automationValidation\.recurringRuns\}/);
  assert.match(source, /15-minute schedule, a 24-hour campaign/);
  ...
  assert.match(snapshot, /"documentedRuns": 9/);
  assert.match(snapshot, /"recurringRuns": 0/);
});
