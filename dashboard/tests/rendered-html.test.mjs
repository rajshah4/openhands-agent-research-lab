...
test("includes the Agent Canvas deployment evidence", async () => {
  ...
  assert.match(source, /Controller placement and durable state/);
  ...
  assert.match(source, /Tested controller configurations and operating limits/);
  assert.match(source, /where can the controller run, and\s+how much concurrent work was independently verified/);
  ...
  assert.match(source, /External service/);
  assert.match(source, /Enterprise automation/);
  assert.match(source, /Beside Agent Canvas/);
